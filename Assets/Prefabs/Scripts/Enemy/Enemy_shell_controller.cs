...
public class Enemy_shell_controller : MonoBehaviour
{
    ...
    public void ShellInitialization(Vector2 direction_, float speed_, float lifetime_, float damage_)
    {
    ...
    IEnumerator DesroyAnimation(float delay)
    {
        ...
        animator.SetTrigger("Trigger_destroy_boss");
    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
        if (!shell_enable)
        {
            shell_enable = true;
            StartCoroutine(DesroyAnimation(0.2f));
            if (collider.gameObject.CompareTag("Player") || collider.gameObject.CompareTag("Destroying_object"))
            {
